...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="oai:ListRecords/oai:record/oai:metadata/oai_dc:dc">
        ...
        <xsl:variable name="prefNameDirect">
            ...
            <xsl:for-each select="dc:creator.name.preferred">
                <xsl:choose>
                    <xsl:when test="contains(., ',')">
                        <xsl:value-of select="replace(., '(.*), (.*)', '$2 $1')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="."/>
                    </xsl:otherwise>
        ...
        <xsl:variable name="fullNameDirect" as="node()*">
            <xsl:for-each select="dc:creator.name.full">
                <xsl:value-of select="replace(replace(dc:creator.name.full, '( [A-Z])( |$)', '$1.$2'), '^(.*), (.*)$', '$2 $1')"/> <!-- TODO or should there be a . instead of dc:creator.name.full in the replace? -->
            </xsl:for-each>
        </xsl:variable>
        ...
        <xsl:variable name="fullNameInverted" as="node()*">
            <!--<xsl:choose>
                <xsl:when test="matches(dc:creator.name.full, ' [A-Z]( |$)')">
                    <xsl:value-of select="concat(dc:creator.name.full, '.')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="dc:creator.name.full"/>
                </xsl:otherwise>
            </xsl:choose>-->
        ...
        <marc:record>
            ...
            <marc:datafield tag="245" ind1="1" ind2="{$ind2}">
                ...
                <!-- Insert name(s) in direct order, using preferred form if available TODO: this should be as it appears on document -->
            ...
            <xsl:if test="count($fullNameInverted)>1">
                <xsl:for-each select="$fullNameInverted[position()!=1]"> <!-- TODO use not() instead of != ??? -->
